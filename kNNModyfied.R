...
subclass <- function(xl,type) {   #выбор определённого вида цветов
  n <- dim(xl)[2]
...
kNNMod <- function(xl,z,k) {   #функция выбора класса методов kNN
  ...
  class <- types[which.min(classDist)]     #выбирает тот вид, у которого "средний" объект ближе всего к z
  return (class)
}
determKNNMod_LOO <- function(xl) {   #определение оптимального k методом LOO
  ...
  text(optK+2,LOO[optK,2]+0.05,labels=res)
  return (optK)
}
...
main <- function() {
  ...
  optK <- determKNNMod_LOO(xl)   #находим оптимальное k
  cnt <- 1
  for (i in 1:yLen) {
    print(Y[i])
    for (j in 1:xLen) {
      ...
      flowers[j,i] <- kNNMod(xl,z,optK)   #находим предположительный класс методом kNN
      cnt <- cnt+1
    }
  }
  plot(positions,pch=1,bg="white",col=colors[flowers],asp=1,main="Map of kNNMod for optimal K",xlab=xname,ylab=yname)
